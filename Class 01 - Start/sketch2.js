...
/* function drawTicks(_numTick, _rotateNum=0) {
...
function drawAxis(_pos, _rotationAngle, _length, _strokeColour, _strokeWeight, _numTicks, _tickLength) {
   
    translate(_pos.x, pos.y);
    rotate(_rotationAngle);
    stroke(_strokeColour);
    strokeWeight(_strokeWeight);
    line(0,0, _length,0)

    let tickGap = _length/(_numTicks);

    for(let x=0; x < _numTicks+1; x++) {
        line(x*-tickGap,0, -_tickLength,x*-tickGap)
    }
}
...
{ 
    ...
    drawAxis(createVector(20,400), 45,400,150,1,5,15);
